...
### ポインター（三角矢印）
- DESIGN_REFERENCE セクション1 のデザイン
- heading 連動で進行方向を向く
- AdvancedMarker, zIndex=100
- 色は PositionQuality に応じて変化（active: 青、lost: 青点滅、denied: 赤）

### パルスリング（精度円）
- accuracy メートル値を地図のズームレベルに応じた半径のピクセル数に変換して描画
- 半透明の円（bg-blue-400/20、border: blue-400/40）
- pointer-events: none でクリック透過
- accuracy が null の場合は非表示

### 道路スナップ（D-027）
- 保存済みルートポリライン上の最近点にポインターをスナップ表示
- パルスリングは GPS の生座標を中心に表示（スナップしない）
- そのためポインターとパルスリングの中心がずれることがある
- ルートからの距離が逸脱判定閾値（50m）を超える場合はスナップしない（生座標のまま表示）

### GPS アイコン（ナビヘッダー内）
- 「ナビゲーション中」テキストの右側に配置
- PositionQuality に応じた表示:
  - active: GPS アンテナアイコン + 精度 m 表示（例: 「GPS 5m」）
  - lost: GPS アンテナアイコン（警告色）+ lost 継続秒数表示（リアルタイム更新、例: 「GPS 45s」）
  - denied: GPS アンテナアイコン（赤）+ 拒否表示
- クリック時のポップオーバー:
  - lost: 「GPS信号を受信できません（{N}秒経過）」
  - denied: 「位置情報の使用が許可されていません。ブラウザの設定から位置情報を許可し、ナビを再開始してください」

### denied 状態の再試行
- denied 検知中は 5 秒間隔で getCurrentPosition を再試行する
- 成功したら watchPosition に切り替え、再試行インターバルを停止する

### 適応的 lost 閾値（D-028）
- 直近10回の watchPosition 更新間隔をリングバッファに記録
- lost 閾値 = median(リングバッファ) × 3
- 下限 10 秒、上限 120 秒
- 初回（10回未満）: デフォルト 15 秒
- ナビ開始時にリングバッファをクリア
...
#### 1. GPS 精度情報を活用した動的スナップ閾値

現在のポリラインスナップ閾値は固定 50m（SNAP_THRESHOLD_M）。
Geolocation API の accuracy（メートル）を動的に活用することで、スナップの ON/OFF 判断を改善できる。

- 精度良好時（accuracy 5m）に閾値 50m は広すぎる → 本当に逸脱しているのにスナップしてしまう
- 精度悪化時（accuracy 30m）に閾値 50m は狭すぎる → GPS が不正確なだけなのにスナップが外れマーカーが道路から飛び出す

動的閾値の方針: `snapThreshold = max(accuracy × N, MIN_THRESHOLD)` のような計算で、精度に応じて閾値を伸縮させる。N の値と MIN_THRESHOLD は Step 3 実装時に決定する。

#### 2. ポリラインスナップ OFF 時の Snap to Roads API 活用

ポリラインスナップが外れた（= 逸脱判定された）状態では、マーカーは GPS の生座標に表示される。
生座標は道路から数メートル〜数十メートルずれるため、ユーザーに「ルートを外れた」だけでなく「道路も外れた」ように見えてしまう。

Google Roads API（Snap to Roads）を散発的に呼び出すことで、逸脱中でもマーカーを道路上に維持できる。
...
コスト試算: 1時間のナビで逸脱10回、各30秒として約70リクエスト/時間。月2,100リクエスト程度。

Phase 1 では API キーがクライアント露出のため導入不可。Phase 2（サブスクリプション + サーバーサイド API キー管理）で導入を検討する。
...
GPS は watchPosition（enableHighAccuracy: true）1本で運用する。
2系統並走方式は廃止済み（D-019 参照）。

### 地図初期表示（ルート編集画面、ウェイポイントなしの場合）

ルート編集画面では Geolocation API を一切呼ばない（D-020）。
