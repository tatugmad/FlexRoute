...
## D-036: free モードのカメラ制御（heading 回転 + エッジ追従）

- **決定**: followMode=free でも headingUp なら地図の heading を更新する。また、現在地マーカーが画面端に接近した場合、マーカーの進行方向に余裕を持たせた位置にスクロールする（エッジ追従）
- **heading**: auto と同じ計算で heading を moveCamera に渡す。northUp 時は heading を変更しない
- **エッジ追従**: bounds の端からマージン 60px 以内にマーカーが入った場合に発動。center を進行方向の反対側にオフセットして、前方の視野を確保する
- **center 制御の違い**: auto は常に currentPosition を center にする。free はユーザーのドラッグ位置を尊重し、マーカーが端に来たときだけスクロールする
- **理由**: v1.6.84 で followMode !== "auto" 時に moveCamera を完全スキップしたため、free + headingUp で地図が回転しなくなった。ドラッグの自由は維持しつつ、heading 回転とマーカーの画面内保持を両立する

## D-037: CameraController — Google Maps カメラ API 一元化

- **決定**: NavMapController / NavWheelZoom / ZoomInOutButtons に分散していた Google Maps カメラ API 呼び出しを、CameraController シングルトンサービス（src/services/cameraController.ts）に一元化する
- **設計原則**: SensorBridge（D-029）と同じ — UI は意図（what）を伝え、CameraController が実装（how）を決める
- **panTo 復活**: northUp + auto モード（heading 変化なし + zoom 変化なし）の場合のみ panTo を使用し、滑らかなセンター追従アニメーションを実現。headingUp モードでは moveCamera を維持（D-032 の教訓: panTo と heading の個別適用はカクつく）
- **window.__wheelMode 廃止**: CameraController の内部状態に移動。カスタムイベント 'wheelmode-changed' も廃止
- **pivotZoom 内包**: NavMapController.tsx から export していた pivotZoom を CameraController の private メソッドに移動
- **NavMapController → NavCameraSync リネーム**: 責務が「カメラ制御」から「store → cameraController への橋渡し」に変わったため名前を変更
- **却下案**: useCamera フック方式 → map インスタンスのライフサイクル管理が React レンダーに結合され、ZoomInOutButtons の長押しチェーン（idle イベントベース）と相性が悪い。シングルトンサービスなら任意のタイミングで呼べる
- **v1.6.93**: 理想インターフェースに再設計。呼び出し側は外部トリガー（onPositionUpdate, onWheel, onZoomButtonDown/Up）のみ伝え、CameraController が全内部判断（auto/free 分岐、heading 補間、scrollwheel 制御、debounce、加速テーブル、idle チェーン、autoZoom 計算、余韻カット）を行う。モード切替インフラを導入し CameraMode インターフェースを定義、mode ファイル単位で方式を差し替え可能にした。NavWheelZoom を廃止し CameraController に吸収。useAutoZoom を廃止し CameraController に吸収
- **v1.6.94**: 5 つのカメラモード（ORG, SETTER, SET+PAN, MOVE, MOVE+TW）を実装。Google Maps API のカメラ操作系統ごとに画一的な構成で比較評価。P/N ホイールモードトグルを廃止し、モード選択 UI（CameraModeSelector）を配置
- **v1.6.95**: MOVE+TW の Tween.js v25 互換性修正（Group API への移行）
- **v1.6.96**: marker-move + head-up で回転が地図中心で行われる問題を修正。回転ピボット補正関数 calcRotationPivotCenter を utils.ts に追加。全 5 モードの marker-move ブランチに適用。MOVE+TW の marker-move に Tween 補間を追加
- **v1.6.97**: MOVE+TW のドラッグ引き戻しバグ修正。CameraMode に onDragStart を追加し、ドラッグ開始時に進行中の Tween を停止
- **v1.6.98**: heading-master 方式の v2 モード 5 つ（ORG2, SETTER2, SET+PAN2, MOVE2, MOVE+TW2）を追加し全 10 モード体制。heading を master、center を slave にし、毎フレーム heading から center を導出する deriveCenter 関数を導入。回転中のマーカー振り回し解消を検証した結果、**MOVE+TW2 でのみ完全解消**を確認。SETTER2 / SET+PAN2 は heading_changed イベントが毎フレーム発火しないため不十分

## D-038: Google Maps API 依存の段階的抽象化方針
- **決定**: 各コンポーネントは外部 API を直接呼ばず、意図（what）をサービス層に伝える。サービス層が実装（how）を担当する。この原則に基づき、Google Maps API への依存を段階的にサービス層に集約する
- **理由**: 将来の地図プロバイダ移行（Mapbox, MapLibre 等）時に修正を最小限にするため
- **現状の抽象化レイヤー**:
| レイヤー | 意図（what） | 実装（how） | 状態 |
|---|---|---|---|
| CameraController (D-037) | 地図を追従させたい / ズームしたい | panTo, moveCamera, setZoom 等 | ✅ v1.6.93 |
| SensorBridge (D-029) | GPS 位置がほしい | watchPosition / sim パッチ | ✅ v1.6.43 |
| StorageService (D-004) | ルート/ラベル/場所を保存したい | localStorage（→ Phase 2: PostgreSQL） | ✅ v1.4 |
| FlightRecorder (D-033) | イベントを記録したい | リングバッファ + console フィルタ | ✅ v1.6.43 |
| routeApi.ts | ルートを計算したい | Google Routes API v2 REST | ✅ v1.3 |
| placeDetailsService.ts | 場所の詳細がほしい | Google Places API | ✅ v1.5 |
- **未抽象化の依存カテゴリ**:
| カテゴリ | 依存内容 | 該当ファイル数 | 方針 |
|---|---|---|---|
| レンダリング | Polyline, AdvancedMarker, Map コンポーネント | 約10ファイル | @vis.gl/react-google-maps がアダプタ役。移行時はライブラリごと差し替え |
| ジオメトリ計算 | decodePath, computeDistanceBetween, computeHeading | 6ファイル | Phase 2 で geometry サービスに集約を検討 |
- **Phase 2 判断事項**: 地図プロバイダ移行の具体的検討時に MapAdapter 層の要否を判断する

## D-039: heading-master / center-slave カメラ制御方式
- **決定**: 地図の回転（heading 変更）時に center を heading の従属変数として毎フレーム導出する。center を独立にアニメーションしない
- **理由**: heading と center を独立に補間すると、中間フレームで数学的整合が崩れ、マーカーが画面上で振り回される
- **導出関数**: utils.ts の deriveCenter()。回転ピボット + GPS 移動の線形補間を合成する
- **検証結果**:
  - MOVE+TW2（Tween + moveCamera + heading-master）: **回転中マーカー振り回し完全解消**。requestAnimationFrame ループ内で heading 補間と center 導出が同じフレームで同期的に実行されるため、1フレームも漏れがない
  - SETTER2 / SET+PAN2（setHeading + heading_changed イベント）: 振り回し残存。heading_changed はアニメーション中に毎フレーム発火する保証がなく、イベント間のフレームで heading と center が不整合になる
  - ORG2 / MOVE2（moveCamera 即時）: 中間フレームが存在しないため振り回しは発生しないが、アニメーションもない
- **結論**: heading-master 方式は「自前フレーム補間（Tween + requestAnimationFrame）」と組み合わせたときのみ完全に有効。Google Maps ネイティブアニメーション（setHeading）の heading_changed イベント依存では不十分
- **却下案**: center と heading を独立に Tween 補間 → 中間フレームで不整合が発生し振り回される（v1 の MOVE+TW で確認）

## D-040: setHeading にはアニメーションがある（D-032 改訂）
- **発見**: setHeading() はドキュメントに記載されていないが、ベクターマップでは滑らかな回転アニメーションを持つ。v1.6.94 の SETTER / SET+PAN モードで確認
- **D-032 衝突の解消**: D-032 で setHeading() が @vis.gl/react-google-maps ライブラリの内部状態と衝突する問題が報告されていた。これは当時 `<Map>` の heading prop を同時使用していたため。v1.6.93 で heading prop を完全除去した結果、衝突が解消し setHeading() が正常に動作するようになった
- **影響**: SETTER / SET+PAN 系モードで setHeading() のネイティブアニメーションが利用可能。ただし heading_changed イベントの発火頻度が不十分なため、回転ピボット補正のフレーム同期には使えない（D-039）
