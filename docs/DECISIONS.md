...
- **ピボット計算**: `newCenter = marker + (oldCenter - marker) × 2^(oldZoom - newZoom)` でマーカーの画面上ピクセル位置を不変に保つ。setZoom + setCenter の2呼び出しでアニメーション付きズームを実現
...
- **比較用トグル**: ZoomInOutButtons に "P"(pivot-fine) / "N"(native) の切替を保持。将来の動作遅延調査時にネイティブ動作と比較可能
