...
/** Mode SET+PAN2: setter+panTo ベース + heading_changed で heading-master 方式。 */
export class ModeSetterPan2 implements CameraMode {
  ...
  applyPosition(map: google.maps.Map, pos: { lat: number; lng: number },
    ...
    if (followMode === "auto") {
      ...
      // heading-master: set heading, derive center for rotation correction, then panTo for GPS move
      ...
      // heading_changed リスナーで中間フレーム補正（modeSetter2 と同一）
