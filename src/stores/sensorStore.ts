...
import type { SensorChannelModes, SensorMode, SimValues, PositionCallbackMode } from "@/types";
type SensorStoreState = {
  ...
  setPositionCallbackMode: (mode: PositionCallbackMode) => void;
  setHeadingSync: (sync: boolean) => void;
  setSpeedSync: (sync: boolean) => void;
...
export const useSensorStore = create<SensorStoreState>((set) => ({
  ...
  setPositionCallbackMode: (mode) =>
    set((state) => ({
      simValues: { ...state.simValues, positionCallbackMode: mode },
    })),
  setHeadingSync: (sync) =>
    set((state) => ({
      simValues: { ...state.simValues, headingSync: sync },
    })),
  setSpeedSync: (sync) =>
    set((state) => ({
      simValues: { ...state.simValues, speedSync: sync },
    })),
