...
指示ファイルがある場合:
1. `git checkout main && git pull` を実行し、`_tasks/pending/` の最新状態を取得する
2. 番号が最も小さいものを1件読み込む
3. 内容をユーザーに要約し、「この指示を実行してよいですか？」と確認する
4. **ユーザーの承認なしに実行しない**
5. 承認されたら、作業を実施する。作業ブランチは指示ファイルの内容に従う（新規ブランチ作成、既存ブランチへの追加修正、mainへの直接commit等）
6. 作業完了後、mainに切り替え、指示ファイルの「実行結果」セクションを記入する
