...
3. **ユーザーの承認なしに実行しない**
4. 承認されたら、作業成果物はブランチを作成して実装する（通常のブランチ・PR運用に従う）
5. 作業完了後、mainに切り替え、指示ファイルの「実行結果」セクションに実行日時・結果・成果物（ブランチ名、PR URL等）・問題や備考を追記する
6. 指示ファイルを `_tasks/review/` に移動し、mainに commit & push する
