...
本リポジトリの `_tasks/pending/` に指示ファイルが配置される。セッション開始時に確認すること。

**重要: `_tasks/` 内のファイル操作（pending→review移動、実行結果の追記）はmainブランチに直接 commit & push する。** 作業成果物（コード変更）は通常通りブランチ作成→PRで行う。タスクの状態管理がPRマージ待ちでブロックされないようにするため。

指示ファイルがある場合:
1. `git checkout main && git pull` を実行し、mainの最新状態に切り替える（前回セッションのブランチが残っていても確実にmainから開始する）
2. 番号が最も小さいものを1件読み込む
3. 内容をユーザーに要約し、「この指示を実行してよいですか？」と確認する
4. **ユーザーの承認なしに実行しない**
5. 承認されたら、作業成果物はブランチを作成して実装する（通常のブランチ・PR運用に従う）
6. 作業完了後、mainに切り替え、指示ファイルの「実行結果」セクションに実行日時・結果・成果物（ブランチ名、PR URL等）・問題や備考を追記する
7. 指示ファイルを `_tasks/review/` に移動し、mainに commit & push する
8. ユーザーに「Claudeチャットに結果を確認してもらってください」と伝える
9. 次のファイルがあれば「次の指示があります。続けますか？」と確認する

### 完了報告の形式

タスク完了時には、以下の形式で報告すること:
...
### 呼称変更とコードの関係
ドキュメント上のモード名・機能名の呼称は設計過程で変更されることがある（例: center-auto → marker-lock）。
呼称変更があっても、既存コード内の変数名・文字列値（例: followMode の `"auto"`, `"free"`）は変更しない。
コード値のリネームは、呼称が完全に固まりユーザーから明示的に指示があった場合にのみ行う。
ドキュメントではコード値との対応を用語定義テーブルで管理する（例: SPEC_FEATURES.md のナビゲーション用語定義テーブル）。
...
#### 追従モード
- marker-lock（コード: followMode="auto"）: 現在地を中心に自動追従。ユーザー操作検知で marker-move に遷移
- marker-move（コード: followMode="free"）: ユーザーが自由に地図操作。ボタン操作で marker-lock に戻る

#### ズーム制御
- zoom-auto（コード: zoomMode="autoZoom"）: dynamic zoom が有効。速度・ターン接近で自動ズーム
- zoom-lock（コード: zoomMode="lockedZoom"）: ユーザー手動設定のズームを固定

#### ヘッディング制御
- head-up（コード: headingMode="headingUp"）: 進行方向が上になるよう地図回転
- north-up（コード: headingMode="northUp"）: 常に北が上（デフォルト）
